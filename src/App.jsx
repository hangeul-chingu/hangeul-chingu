...
async function evaluateFile(file, level) {
  ...
  try {
    ...
    const evalSys = isAdv
      ? `너는 한국어 쓰기 평가 전문가 AI '마중'이야. TOPIK 5~6급 기준으로 따뜻하고 구체적인 피드백을 줘.
1. [잘한 점 🌟] 1~2가지 칭찬
2. [개선 포인트 💡] 핵심 2~3개만
3. [수정 예시 ✍️] 학습자 문장 직접 교정
4. [응원 메시지 💪] 동기부여로 마무리`
      : `너는 한국어 쓰기 평가 전문가 AI '마중'이야. TOPIK 3~4급 기준으로 따뜻하고 구체적인 피드백을 줘.
1. [잘한 점 🌟] 1~2가지 칭찬
2. [개선 포인트 💡] 핵심 1~2가지만
3. [수정 예시 ✍️] 학습자 문장 직접 교정
4. [응원 메시지 💪] 동기부여로 마무리`;
  ...
  if (mode==="submit") return (
    <div style={{padding:"8px 0"}}>
      ...
      <div style={{background:"white",borderRadius:16,padding:"14px 16px",boxShadow:"0 4px 18px rgba(0,0,0,.07)",marginBottom:12}}>
        ...
        <button onClick={()=>fileRef.current?.click()} style={{width:"100%",border:`2px dashed ${C.purple}66`,borderRadius:12,padding:"22px 16px",background:submitFile?"#F5F0FF":"#fafafa",cursor:"pointer",textAlign:"center"}}>
          {submitFile?<div><div style={{fontSize:24,marginBottom:4}}>✅</div><div style={{fontSize:14,fontWeight:700,color:C.purple}}>{submitFile.name}</div></div>:<div><div style={{fontSize:32,marginBottom:6}}>📁</div><div style={{fontSize:14,fontWeight:700,color:"#777"}}>파일 선택 (사진·PDF·TXT)</div></div>}
        </button>
        <button onClick={handleSubmit} disabled={!submitFile||submitLoad} style={{marginTop:12,width:"100%",background:`linear-gradient(135deg,${C.pink},${C.orange})`,color:"white",border:"none",borderRadius:50,padding:"14px 0",fontSize:15,fontWeight:900,cursor:"pointer",opacity:!submitFile||submitLoad?0.4:1}}>
          {submitLoad?"AI가 읽는 중... 📖":"✅ 평가 받기"}
        </button>
      </div>
      {submitFeed&&<div style={{background:"white",borderRadius:18,padding:16,boxShadow:"0 4px 18px rgba(0,0,0,.07)"}}>{renderFeedback(submitFeed,C.teal)}</div>}
    </div>
  );
            ...
            <div style={{marginBottom:12}}>
              ...
              {showTopics&&TOPICS.map((t,i)=>(
                <button key={i} onClick={()=>{const a=[...wText];a[0]=t.hint;setWText(a);setShowTopics(false);}} style={{width:"100%",background:`${C.orange}0D`,border:`1.5px solid ${C.orange}55`,borderRadius:12,padding:"9px 12px",cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                  ...
                  <div><div style={{fontSize:13,fontWeight:800,color:C.orange}}>{t.title}</div><div style={{fontSize:11,color:"#888"}}>{t.hint}</div></div>
                </button>
              ))}
            </div>
          )}
