...
[이모지] 🎈😊🌟`,
  ...
  write:{
    mid:[
      ...
      "따뜻한 글쓰기 코치. TOPIK 3~4급. 생각 단계: 나는~라고 생각해요 형태. 중급 대안 제시. 칭찬+이유 연결. 3문장 이내.",
      "따뜻한 글쓰기 코치. TOPIK 3~4급. 이유 단계: 왜냐하면~이기 때문이에요. 중급 발전 표현 1가지. 3문장 이내.",
    ],
    adv:[
      "따뜻한 글쓰기 코치. TOPIK 5~6급. 현상 단계: 사회적 맥락 2~3문장. 고급 어휘 권장. 4문장 이내.",
      "따뜻한 글쓰기 코치. TOPIK 5~6급. 생각 단계: 관용구·고급 어휘로 논리적 의견. 4문장 이내.",
      "따뜻한 글쓰기 코치. TOPIK 5~6급. 이유 단계: 따라서·이로 인해 활용. 사자성어·관용구 제안. 4~5문장.",
    ],
  },
...
function SpeakTab({level, unlock, speaking, speak}) {
    ...
    <div style={{padding:"8px 0"}}>
      ...
      <div style={{background:"white",borderRadius:16,padding:"14px 16px",boxShadow:"0 4px 18px rgba(0,0,0,.07)",marginBottom:10}}>
        ...
        <button onClick={()=>fileRef.current?.click()} style={{width:"100%",border:`2px dashed ${C.purple}66`,borderRadius:12,padding:"22px 16px",background:submitFile?"#F5F0FF":"#fafafa",cursor:"pointer",textAlign:"center",WebkitTapHighlightColor:"transparent"}}>
          {submitFile ? (
            <div><div style={{fontSize:24,marginBottom:4}}>✅</div><div style={{fontSize:14,fontWeight:700,color:C.purple}}>{submitFile.name}</div><div style={{fontSize:11,color:"#aaa",marginTop:2}}>{(submitFile.size/1024).toFixed(0)} KB · 탭하면 다른 파일로 교체</div></div>
          ) : (
      ...
      <div style={{background:"white",borderRadius:16,padding:"14px 16px",boxShadow:"0 4px 18px rgba(0,0,0,.07)",marginBottom:12}}>
        ...
        <div style={{fontSize:13,color:"#666",lineHeight:1.6,paddingLeft:38,marginBottom:12}}>
          한국어로 쓴 글의 <strong>AI 평가</strong>를 원하신다면<br/>아래 버튼을 눌러주세요!
        </div>
...
export default function App() {
  ...
  if (!level) return (
    <div onClick={unlock} style={{minHeight:"100vh",background:`linear-gradient(150deg,${C.bg},#FFF0F9 50%,#F0FFFE)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <div style={{fontSize:26,fontWeight:900,color:"#333",marginBottom:6}}>한글 친구</div>
      <div style={{fontSize:14,color:"#888",marginBottom:36,textAlign:"center",lineHeight:1.7}}>시작하기 전에 한국어 수준을 선택해줘요!</div>
      {[
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(150deg,${C.bg},#FFF0F9 50%,#F0FFFE)`,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      <div style={{background:`linear-gradient(100deg,${C.pink},${C.orange},${C.yellow})`,padding:"16px 16px 12px",position:"relative"}}>
        <div style={{textAlign:"center"}}>
          ...
          <div style={{color:"rgba(255,255,255,.85)",fontSize:12,marginTop:2}}>Korean Speaking & Writing Trainer</div>
        </div>
        <button onClick={()=>setLevel(null)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"rgba(255,255,255,.22)",border:"1.5px solid rgba(255,255,255,.6)",borderRadius:20,padding:"4px 10px",cursor:"pointer",color:"white",fontSize:11,fontWeight:700,WebkitTapHighlightColor:"transparent"}}>
          {level==="adv"?"🔥 고급":"🌱 중급"} ✕
        </button>
      </div>
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"12px 12px 80px",boxSizing:"border-box"}}>
        ...
        {tab==="speak"&&<SpeakTab level={level} unlock={unlock} speaking={speaking} speak={speak}/>}
        {tab==="write"&&<WriteTab level={level}/>}
        {tab==="tutor"&&<TutorTab level={level}/>}
      </div>
    </div>
  );
}
